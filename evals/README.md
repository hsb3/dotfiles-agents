...
`package_toolbox.py` builds a new, isolated deployment directory from the marketplace
manifest, workflow guide, deploy wrapper, and an explicit Vite production build. It writes the wrapper,
`pb_public/` assets, private deterministic catalog/package receipts, and the PocketBase hook;
...
For a local package check from the repository root:
...
hash inventory records UI source/build and catalog inputs; it does not by itself prove how the build was produced.
Deploy and recovery procedures are in [PROCEDURES.md](PROCEDURES.md#procedure-update-the-existing-toolbox).
...
| `assessments` | one verdict: extender × framework element × assessor | `verdict` (present / partial / absent / not-applicable), `evidence`, `score` (optional), `assessor`, optional `eval_run` (current campaign association, not immutable assessment history) |
...
normalized for agent-usage analytics. Loaded by `load_harness_runs.py`. Candidate names do not
establish a relation to database extenders or packaged plugins; only explicit record relations
support navigation between areas.
...
`runs.measurement` and `eval_responses.measurement` carry versioned availability and source
provenance. Read a scalar measurement only when its availability flag is true: PocketBase number
and boolean defaults cannot distinguish an omitted value from observed zero or false. A legacy
record without this envelope is unverified, even if it has a scalar value. Execution validity is
separate from the grading outcome; a failed grade can belong to a valid execution. Process errors,
skips and insufficient source evidence cannot establish a compatible measured comparison.

Pairwise deltas describe the selected observations with their sample counts and units; they do
not establish causality, statistical significance or current installed behavior. Preserve missing
source revisions and unspecified timestamp timezones. Artifact `run` is its first owner, not an
exhaustive membership relation: include artifacts referenced by the selected run's events and
tool calls when browsing evidence. Missing local logs do not mean stored evidence never existed.
