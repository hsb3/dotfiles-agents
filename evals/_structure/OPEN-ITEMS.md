...
| Id | Item | Notes |
|---|---|---|
| EDB-26 | `load_coverage.py` needs a scoped/delta mode | Found in the W2 delta pass (2026-07-21): the loader demands the full extender cross product and re-stamps `eval_run` on every carried-forward row, silently rewriting provenance for extenders the pass never judged (864 rows). W2 restored the original linkage from the git-committed pre-pass `data.db` (verified 0 stragglers) — that restore step is mandatory for every delta pass until the loader accepts a scoped extender list (natural home: the M2/#162 tooling wave). Post-restore `--dry-run` permanently reports those rows as pending updates — expected; never run it for real. |
