...
| Id | Item | Notes |
|---|---|---|
| EDB-23 | Description-only judging of externals is under-determined | M1 evidence: `typescript-lsp` (22-char description) got two disjoint single-job mappings from two competent judges (implement-change vs orient-codebase); the union was accepted at adjudication. **Progress 2026-07-21 (W3, #167):** all 5 externals' `provides` fields enriched from pinned-ref upstream content (83–90 words each, capability + trigger + exclusions) and ingested. Key finding: typescript-lsp is a declarative LSP registration with NO invokable surface — which explains the M1 divergence. **Remaining:** re-judge the 5 externals' `coverage-v1` rows under the enriched descriptions (scoped `--extenders` delta; typescript-lsp first — expect orient-codebase partial to firm up, implement-change to weaken; the other 4 have no strong prior-change hypothesis). Fold into the next judged pass (natural home: the M4/#164 v2 pilot). |
...
| EDB-19 | Wire M5 execution to Henry's existing meta-harness (reuse, not build) | Henry already has a meta-harness for testing across opencode + other coding-agent harnesses (2026-07-20) — **do NOT build an `opencode_exec` SkillOpt backend.** Get the repo/path pointer (likely the same as EDB-14), understand its interface, and route M5's with-skill/without/baseline rollouts through it on cheap/free models. `opencode-expertise` covers the CC→opencode translation (hooks the main gap). **Pointer resolved 2026-07-22 (owner ruling, EDB-14): the harness is the root `harness/` in THIS repo (PR #169) — see `harness/README.md` for its interface + extraction checklist. #165 UNBLOCKED.** Remaining here = the actual wiring (route M5 rollouts through `harness/`), which IS the #165 build; this item tracks it until #165 lands. |
| EDB-16 | Registry expansion — research follow-ups | From the EDB-15 research pass's open questions: (a) vet whether Microsoft / Firebase / Supabase (on skills.sh leaderboards) ship their own eval suites — **partially answered**: Microsoft ships SkillOpt (now framework `skillopt`) + hve-core (Copilot); (b) `coleam00` was added because we vendor from it but was NOT independently vetted — run a real vetting pass — **update 2026-07-20:** the M1 dup-check confirmed the vendored skill duplicates our authored `excalidraw` (M3 drop/fold candidate), which may moot the vetting; **outcome 2026-07-21 (#159): mooted by drop, with a residual-value caveat** — the entry was removed from `externals.yaml`, but the vetting comparison found two upstream capabilities our authored skill lacks (headless Playwright render→fix loop; a diagram-design methodology layer) — tracked as an enhancement candidate for the authored `excalidraw`, and the `sources.coleam00` publisher disposition goes to the Wave-2 DB pass; (c) no publisher ships an OBJECTIVE cross-publisher benchmark leaderboard — track if one emerges. |
...
| Id | Item | Outcome |
|---|---|---|
| EDB-14 | Built-in agents / meta-harness from Henry's other repo | **Resolved 2026-07-22 (owner ruling).** The meta-harness pointer resolves to the root **`harness/`** in THIS repo (delivered PR #169), not a separate repo — a self-contained uv project that drives Claude Code **or** opencode headlessly against fixture workspaces, two-tier grades (deterministic `check.py` + pinned-Haiku rubric), and appends to an append-only ledger. It is the exact EDB-14/19 execution layer; interface + extraction checklist in `harness/README.md`. Consequence: #165/M5 UNBLOCKED, CHARTER decision 8 corrected in place, EDB-19's remaining wiring is the #165 build. |
| EDB-26 | `load_coverage.py` needs a scoped/delta mode | **Resolved 2026-07-21 (W3, #162).** Found in the W2 delta: unscoped loads re-stamp `eval_run` on every carried row (864 rows' provenance forged; restored from the git-committed pre-pass `data.db`, 0 stragglers — restore pattern recorded in the W2 adjudication run notes). Fix landed: `--extenders slug1,slug2` scoped mode validates the input against exactly the named set and never touches other extenders' rows. Live-proven: scoped dry-run on deep-research considered 24 rows only, the sole pending update being its one adjudicated row (the documented expected-N pattern). PROCEDURES gotcha updated. |
