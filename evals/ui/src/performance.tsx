...
import { artifactByteSize, assessmentsForCampaign, campaigns, campaignById, evidenceForRun, eventsForRun, protectedArtifactUrl, responsesForCampaign, runById, runs, sortRuns, toolCallsForRun, type Run } from "./performance-data";
...
function useLoad<T>(key: string, load: (signal: AbortSignal) => Promise<{ kind: string; data?: T }>) {
  ...
  useEffect(() => {
    ...
    load(controller.signal).then((reply) => {
      ...
      setResult(reply.kind === "ok" ? { state: "populated", data: reply.data } : { state: reply.kind === "access" ? "access" : "error" });
    });
...
export function Performance({ session, onExpired }: { session: Session; onExpired: () => void }) {
  ...
  const runId = recordScope(location.hash, "run"); const campaignId = recordScope(location.hash, "campaign");
  const result = useLoad(`runs:${query}:${config}:${page}:${perPage}:${sort}`, (signal) => runs(session, { query, config, page, perPage, sort: sort === "id" ? "+id" : `+${sort}`, signal }));
  ...
  const rows = useMemo(() => result.data ? (sort === "id" ? result.data.items : sortRuns(result.data.items, sort, "asc")) : [], [result.data, sort]);
