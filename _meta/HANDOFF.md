...
- **Done:** W0 seed · W1 judged pass (6 judges + 2 blind reviewers + session adjudication) ·
  ...
  `eval_runs`/`eval_responses`) · **W9/M1 coverage body (EDB-13 resolved)**. Briefing deck + explainer:
...
- **Done (this session, 2026-07-20, Fable foreman): M1 = the W9 mapping body** (commits
  `9333784` decision layer + `492432d` delivery). All 37 extenders → 24 jobs as 888
  `coverage-v1` assessments (idempotent loader `load_coverage.py`); 46 `relationships`
  (1 opus-confirmed duplicative: excalidraw-diagram-coleam00 ≡ excalidraw → M3; 21 directed
  hand-off links = EDB-22 substrate); 24 `job_coverage` rows — **19 covered / 3 partial /
  2 gap; dispositions 22 author · 1 vendor (improve-code) · 1 reference (operate-browser-ui)**.
  Crew: 7 sonnet judges + 2 blind opus reviewers + opus dup-check + session adjudication
  (10 rows patched, 4 boundary rules → EDB-10). Provenance: eval_runs
  `m1-coverage-{judged,review,adjudication}-2026-07-20`. Matrix: `coverage-matrix.md`
  (generated by `render_matrix.py` — regen after any coverage change, never hand-edit).
  All PLAN DoD gates ran green (incl. `make ci` 120 tests); data.db committed
  WAL-checkpointed; server restarted after.
- **NEXT (in order of readiness):** (a) **owner court — issue #153 +
  `_meta/extender-db/DECISIONS-NEEDED.md`** (EDB-24 promotions, coleam00 drop, vendor
  acceptance, EDB-9 gate path, meta-harness pointer; each item carries a default so a bare
  👍 accepts all) — which promotions get rostered feeds M3; (b) **M4 EDB-21** formalize judge+rubric+CaseScore — the M1 boundary
  rules + dup-check composition-tells doctrine (INSIGHTS §2) are its direct inputs;
  (c) **M2** analysis surface (`report.py`) — `render_matrix.py` is its seed; (d) EDB-9
  (obsidian-cli phantom refs — ready catalog fix, doubles as W6 update proof). New items:
  EDB-23 (externals' descriptions too thin to judge), EDB-24.
