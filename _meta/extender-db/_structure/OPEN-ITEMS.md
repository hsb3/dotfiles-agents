...
| Id | Item | Notes |
|---|---|---|
| EDB-15 | Trusted-source (publisher) registry | New (Henry, 2026-07-20): a `sources` collection of trusted extender **publishers** carrying quality signals (publishes evals? benchmark data? adoption? license?) + a trust tier, that externals link to and W8 comparators are drawn from. Reframes W9 gaps: a gap/external-only job gets a **disposition** (author / vendor / reference / compare), not an automatic "author from scratch." **Schema/IA change — needs Henry's sign-off before build.** Seed via a real research pass (not from memory). |
