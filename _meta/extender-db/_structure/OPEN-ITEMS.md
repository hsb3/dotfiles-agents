...
| EDB-13 | Jobs-to-be-done coverage framework (W9) | **Taxonomy DONE 2026-07-20:** `hsb3-jobs-to-be-done` loaded (24 jobs, 7 families, kind `job-taxonomy`, candidate), authored with Henry at deliverable altitude / all-work scope; idempotence + assessor discipline re-verified. **Storage decided 2026-07-20 (CHARTER decisions 9–10, Henry signed off):** per-job disposition → new `job_coverage` collection; pairwise links → new `relationships` collection. **Remaining (W9 body):** the M1 build per PLAN's execution architecture — schema + `load_coverage.py`, ~6–7 sonnet judge fan-out, 2 opus reviewers, session synthesis (relationships + dispositions) → coverage matrix. |
