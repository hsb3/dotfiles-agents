...
| ---- | ----------------- | ------------------------------ |
| CI aggregate (required) | `make ci` (runs both guards below) on every PR + push to main | always |
...
| Naming taxonomy | `manifests/naming.md` (no auto-lint yet) | naming a new primitive or plugin |
| YAML / workflow lint | `yamllint`; `actionlint` | editing `*.yaml`/`*.yml`; `.github/workflows/` |
