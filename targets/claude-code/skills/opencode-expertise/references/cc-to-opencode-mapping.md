...
_The translation table for the dotfiles-agents pipeline (opencode target). ✅ = verified
...
| **Slash command** (`.claude/commands/*.md`) | Command (`.opencode/commands/*.md`) ✅ | Frontmatter remap + body → `template`-style content; `$ARGUMENTS`/`$1` survive ✅. **Moot for dotfiles-agents** (skills-over-commands decision — commands aren't a primitive). No `.claude/commands` auto-compat ✅. | Mechanical (unused) |
