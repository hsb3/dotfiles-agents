...
| a board status readout + the ask | advisor board readout | pptx-henry | optional | `examples/advisor-board-readout/playbook.md` |
| what a client gets + why to trust it | client product overview | pptx-henry | optional | `examples/client-product-overview/playbook.md` |
...
**pptx-henry** skill. Audience drives toolchain, theme, voice, and how honest framing is
phrased.
...
4. **Author** the source (`slides.json` for deck-builder, `deck.js` for pptx-henry) to the
   playbook's structure, mirroring the sample.
5. **Build**: validate + export (deck-builder) or render + visual-QA (pptx-henry); narrate +
...
- **pptx-henry** - owns the palette, semantic theme tokens, typography, and visual QA for the
