...
def load_snapshot(owner: str, number: int, owner_type: str):
    ...
    pj = graphql("query($o:String!,$n:Int!){ %s(login:$o){ projectV2(number:$n){ id } } }" % scope,
                 o=owner, n=str(number))
    ...
    for f in graphql(fq, id=pid)["data"]["node"]["fields"]["nodes"]:
        if not f:
            continue
        fields[f["name"]] = {
            "id": f["id"], "dataType": f.get("dataType"),
            "options": {o["name"].lower(): o["id"] for o in f.get("options", [])},
            "iterations": {it["title"].lower(): it["id"]
                           for it in ((f.get("configuration") or {}).get("iterations", []) +
                                      (f.get("configuration") or {}).get("completedIterations", []))}
                          if f.get("configuration") else {},
        }

    items, cur = {}, "null"
    iq = """query($id:ID!){ node(id:$id){ ... on ProjectV2 { items(first:100, after:%CUR%){
    ...
    while True:
        page = graphql(iq.replace("%CUR%", cur), id=pid)["data"]["node"]["items"]
        for it in page["nodes"]:
            ...
            for fv in it["fieldValues"]["nodes"]:
                ...
                if not fn:
                    continue
                cur_vals[fn] = (fv.get("name") or fv.get("text") or fv.get("date")
                                or fv.get("title") or fv.get("number"))
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Apply a changeset to a GitHub Project v2 (dry-run default).")
    ...
    for r in rows:
        issue = int(r["issue"]); fld = r["field"].lower(); val = r["value"]
        tag = f"#{issue} {fld}={val or '(clear)'}"

        if fld in ("blocked_by", "blocking"):
            if not args.repo:
                print(f"SKIP  {tag}: --repo required for {fld}"); skipped += 1; continue
            rc, out, _ = gh("api", f"repos/{args.repo}/issues/{val}", "--jq", ".id", check=False)
            if rc != 0:
                print(f"FAIL  {tag}: blocker #{val} not found"); failed += 1; continue
            blocker_id = out.strip()
            if not args.apply:
                print(f"DRY   would POST {fld}: #{issue} <- #{val}"); changed += 1; continue
            rc, _, err = gh("api", "-X", "POST",
                            f"repos/{args.repo}/issues/{issue}/dependencies/{fld}",
                            "-F", f"issue_id={blocker_id}", check=False)
            if rc == 0:
                print(f"SET   {tag}"); changed += 1
            else:
                print(f"FAIL  {tag}: {err.strip()}"); failed += 1
            continue

        if issue not in items:
            print(f"SKIP  {tag}: not on board"); skipped += 1; continue
        fname = resolve_field(fld, fields)
        if not fname:
            print(f"SKIP  {tag}: no field matches '{r['field']}'"); skipped += 1; continue
        fdef = fields[fname]
        if fdef["dataType"] not in SETTABLE:
            print(f"SKIP  {tag}: field '{fname}' ({fdef['dataType']}) not settable via project API"); skipped += 1; continue
        item_id = items[issue]["item_id"]
        current = items[issue]["values"].get(fname)

        # idempotent skip
        if val == "" and current in (None, ""):
            print(f"OK    {tag}: already clear"); unchanged += 1; continue
        if val and current is not None and str(current).lower() == val.lower():
            print(f"OK    {tag}: unchanged"); unchanged += 1; continue

        if not args.apply:
            print(f"DRY   would set {tag} (was {current})"); changed += 1; continue

        try:
            if val == "":
                graphql("mutation($p:ID!,$i:ID!,$f:ID!){clearProjectV2ItemFieldValue(input:{projectId:$p,itemId:$i,fieldId:$f}){projectV2Item{id}}}",
                        p=pid, i=item_id, f=fdef["id"])
            elif fdef["dataType"] == "SINGLE_SELECT":
                oid = fdef["options"].get(val.lower())
                if not oid:
                    print(f"FAIL  {tag}: no option '{val}'"); failed += 1; continue
                graphql("mutation($p:ID!,$i:ID!,$f:ID!,$o:String!){updateProjectV2ItemFieldValue(input:{projectId:$p,itemId:$i,fieldId:$f,value:{singleSelectOptionId:$o}}){projectV2Item{id}}}",
                        p=pid, i=item_id, f=fdef["id"], o=oid)
            elif fdef["dataType"] == "ITERATION":
                iid = fdef["iterations"].get(val.lower())
                if not iid:
                    print(f"FAIL  {tag}: no iteration '{val}'"); failed += 1; continue
                graphql("mutation($p:ID!,$i:ID!,$f:ID!,$o:String!){updateProjectV2ItemFieldValue(input:{projectId:$p,itemId:$i,fieldId:$f,value:{iterationId:$o}}){projectV2Item{id}}}",
                        p=pid, i=item_id, f=fdef["id"], o=iid)
            elif fdef["dataType"] == "DATE":
                graphql("mutation($p:ID!,$i:ID!,$f:ID!,$d:Date!){updateProjectV2ItemFieldValue(input:{projectId:$p,itemId:$i,fieldId:$f,value:{date:$d}}){projectV2Item{id}}}",
                        p=pid, i=item_id, f=fdef["id"], d=val)
            elif fdef["dataType"] == "NUMBER":
                graphql("mutation($p:ID!,$i:ID!,$f:ID!,$nn:Float!){updateProjectV2ItemFieldValue(input:{projectId:$p,itemId:$i,fieldId:$f,value:{number:$nn}}){projectV2Item{id}}}",
                        "-F", f"nn={val}", p=pid, i=item_id, f=fdef["id"])
            else:  # TEXT and anything else
                graphql("mutation($p:ID!,$i:ID!,$f:ID!,$t:String!){updateProjectV2ItemFieldValue(input:{projectId:$p,itemId:$i,fieldId:$f,value:{text:$t}}){projectV2Item{id}}}",
                        p=pid, i=item_id, f=fdef["id"], t=val)
            print(f"SET   {tag} (was {current})"); changed += 1
        except SystemExit as e:
            print(f"FAIL  {tag}: {e}"); failed += 1

    verb = "applied" if args.apply else "planned (dry-run)"
    print(f"\n{verb}: {changed} changed · {unchanged} unchanged · {skipped} skipped · {failed} failed", file=sys.stderr)
