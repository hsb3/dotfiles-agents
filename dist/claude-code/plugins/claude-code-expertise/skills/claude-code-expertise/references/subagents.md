...
Location: `.claude/agents/<name>.md` (project) · `~/.claude/agents/<name>.md` (user). One file,
frontmatter + body. The body is the subagent's **system prompt**.

```markdown
---
name: test-runner              # required; lowercase-kebab; unique
...
- **Omit `tools`** to inherit every tool the main thread has (including MCP tools). Convenient,
  but broad — the subagent can do anything.
