...
| the `checkout-root:` activation key | Codex only | Read by `codex_workers.py` (worker checkout placement) and `activation.py codex-setup` (the Codex writable root), plus `lane-snapshot`'s scan of that root; Claude Code does not read it yet. opencode's activation parser does not read it and the port has no counterpart, so the key would be read by nobody there. Being one-sided, its doctrine prose lives in a `harness:claude-code` block, never in neutral text. |
