...
- **Edit primitives under `primitives-core/` only.** `plugins/`, `PLUGINS.md`, and
...
- **Every distributed plugin ships a README.** Add its source under
  `primitives-core/bundles/<id>/README.md` (bundles/kits) or
  `primitives-core/standalone-readmes/<id>/README.md` (standalone skills); `make build`
  assembles it into `plugins/<id>/README.md` and rolls it into `PLUGINS.md`'s inventory.
  `make ci` fails if a distributed plugin has no README.
...
- `make ci` is the task interface (`make help` lists every target): `check` (roster ↔ disk
  drift, schema + provenance), `build-check` (marketplace regen drift), `test` (stdlib-only
  unit tests, zero-install). A PR that doesn't pass `make ci` locally won't pass it in CI
  either — run it before you push.
