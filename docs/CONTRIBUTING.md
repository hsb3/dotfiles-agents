...
- **Edit primitives under `primitives-core/` only.** `plugins/<id>/` are thin symlink
  ...
  symlinks for everything else, listed in the root `.claude-plugin/marketplace.json`. The
  `dist/` lanes are generated by `scripts/gen_marketplace.py` — never hand-edit them; change
  the source, then `make build` (dist retires with task-3).
