...
**Work the owner's queue; never self-select from the pile** (2026-08-18). On kata the queue is `kata list --label kaneo-status:up-next` (what sat in Kaneo's Up Next at the 2026-09-02 cutover) plus whatever the owner schedules or prioritizes (`kata next`, `kata ready`). Empty means ask the owner to stock it. **Why:** pulling from the pile substitutes an agent's ranking for the owner's, and a session spends itself on something nobody prioritized.
