    ...
    try:
        if event == "PreToolUse":
            ...
            return {"hookSpecificOutput": {"hookEventName": event,
                    "additionalContext": "Session handoff binding: " + str(binding)}}
...
def persist(binding_path, project, body_file, title="Session handoff", labels=(), related=(), predecessor=None):
    ...
    if str(binding_path) != str(cert) + ".binding":
    ...
    if items:
        ...
        if issue["status"] != "open" or issue.get("metadata", {}).get("github_issue"):
            raise ValueError("handoff destination must be an open native card")
        kata(project, "edit", issue["short_id"], "--body", body)
    else:
        ...
        metadata = {"handoff.writer": binding["key"], "handoff.native": native,
                    ...
                    "handoff.worktree": root, "handoff.lifecycle": "active"}
        if predecessor:
            metadata["handoff.predecessor"] = predecessor
    ...
    verified = kata(project, "show", issue["short_id"])["issue"]
    if verified.get("body") != body or verified.get("metadata", {}).get("handoff.writer") != binding["key"]:
...
def main():
    ...
    parser.add_argument("command", choices=["persist"])
