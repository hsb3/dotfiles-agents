...
REMINDER = (
    "plugin-feedback (worker tier): you MAY file a bug directly about any installed "
    "plugin when its observed behavior contradicts the plugin's own stated contract — "
    "run `python3 {reporter} --help`. You MUST NOT file a feature request: draft one "
    "with `--draft` and hand it to your dispatcher to review and file, since the why has "
    "to be tied to a real limitation. Reports go through the reporter, never a "
    "free-handed `gh issue create`."
)
