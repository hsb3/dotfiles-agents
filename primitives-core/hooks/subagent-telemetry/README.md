...
Codex SubagentStop names the parent in `transcript_path`. Measurements use the child transcript validated and stored at SubagentStart, so parent tokens, model and start time cannot be attributed to the worker. Before a child rollout emits its first `token_count`, telemetry records null context with `pending: true`; malformed or unavailable child usage remains an explicit measurement error.
