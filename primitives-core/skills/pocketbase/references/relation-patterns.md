...
python3 scripts/pb_records.py get posts POST_ID \
  --expand "author"
```
...
```bash
python3 scripts/pb_records.py get users USER_ID \
...
python3 scripts/pb_records.py list post_tags \
...
python3 scripts/pb_records.py list comments \
  --filter 'post = "POST_ID"' \
  --sort "-created" \
  --page 1 --perPage 20
```
...
python3 scripts/pb_records.py update posts POST_ID \
  '{"tags+": ["NEW_TAG_ID"]}'
