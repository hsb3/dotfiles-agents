...
`codex-setup` resolves each role from its project profile first, then a current managed global profile, and only bootstraps missing roles locally. It adds project writable roots without touching hook trust. Atelier owns only the roots it computes (the checkout root, default `<git-common-dir>/atelier-codex/checkouts`, plus the common dir's `worktrees`, `objects`, `refs/heads/atelier` and `logs/refs/heads/atelier`); when it rewrites its managed `writable_roots` block, any other root there is kept after its own and named as kept, except the default checkouts path once `checkout-root` moves away from it. A former custom checkout root is kept the same way; remove it by hand. `check --harness codex` is read-only; stale global profiles require explicit `codex-setup --refresh-global`. It reports trust as unverified until checked in native `/hooks`. See the skill for setup and restart steps.
