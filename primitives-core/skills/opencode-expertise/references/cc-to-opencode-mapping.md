...
| **Slash command** (`.claude/commands/*.md`) | Command (`.opencode/commands/*.md`) ✅ | Frontmatter remap + body → `template`-style content; `$ARGUMENTS`/`$1` survive ✅. **Moot for a skills-only marketplace** (a skills-over-commands convention treats commands as non-primitives). No `.claude/commands` auto-compat ✅. | Mechanical (unused) |
