...
`plugins/<id>/` (decision-030 — no build step; nothing generated is tracked). This page is the
...
2. **Edit primitive bodies in `primitives-core/` only.** It is the single canonical source copy of every primitive.
   `plugins/<id>/` are thin symlink assemblies over it (decision-030) — hand-authored
...
The roster — not a directory listing — is the provenance manifest (decision-030); the drift guard
...
- **`origin: authored | sourced | vendored`** is provenance and is immutable per entry.
  ...
  ([decision-028](../docs/decisions/decisions-028-primitives-core-is-self-authored-only-externals-by-reference.md)). Third-party material is
  recorded **by reference** in [`../externals.yaml`](../externals.yaml) (non-null `upstream` +
  `ref`), never copied into the source tree ([decision-026](../docs/decisions/decisions-026-externals-are-tracked-and-cloned-never-vendored.md)).
...
| `make provenance` | **Provenance** — every `primitives-core/` body is `origin: authored`; every `externals.yaml` entry has non-null `upstream` + `ref` (decision-028 / decision-026). |
...
| `make symlinks` | **Symlink-assembly lint** (decision-030) — every link under `plugins/` resolves in-repo; the root marketplace manifest and the assemblies match 1:1. |
...
Nothing generated is tracked (decision-030). If something must be generated (e.g. the opencode
