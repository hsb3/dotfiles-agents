...
| `task-authoring` | The item-body standard the desk delegates to. The desk owns the plan; what a tracker item's own body must contain — acceptance criteria you can check, dependencies and gates, the close-when section an epic carries — is defined here, and the desk points at it rather than restating it. That is also the bar `_utils/conformance.py` reads an item against, so the standard and the gate stay one thing. |
