...
| `checkout-root` | `codex_workers.py`, `activation.py` (Codex setup), `lane-snapshot` | Where automatic Codex worker checkouts land: a relative value resolves against the main checkout, absent/blank keeps the default `<git-common-dir>/atelier-codex/checkouts`, and an invalid value (a list/mapping, a git-less or separate-git-dir/bare project, or a path that resolves, symlinks followed, to the project itself, an ancestor, `$HOME` or inside `.git`) is a hard error rather than a silent fallback. `lane-snapshot` scans the configured root, and Codex setup adds an in-tree root to `.git/info/exclude`. Claude Code subagent worktrees: not yet. |
