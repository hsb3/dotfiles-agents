...
| `lane-snapshot` | hook (`SessionStart`) | Starts a background daemon that commits every agent worktree's working tree to `refs/lane-snapshots/<name>` every few minutes, so a worker's uncommitted output survives a crash or a mistaken prune. A pidfile lock keyed by the git common dir means every session of a repo, from any worktree and either harness, shares one daemon (a bare repository's worktrees keep one each); it exits on its own when its root is gone or after `LANE_SNAPSHOT_TTL` (12 h) with no snapshot written, and `--check` names orphans. Recover with `git show refs/lane-snapshots/<name>:<path>`; check it is actually running with `python3 .../lane-snapshot/snapshot_lanes.py --check`. Adds refs only, never prunes. |
...
| `checkout-root` | `codex_workers.py`, `activation.py` (Codex setup) | Where automatic Codex worker checkouts land: a relative value resolves against the main checkout, absent/blank keeps the default `<git-common-dir>/atelier-codex/checkouts`, and an invalid value (a list/mapping, or a git-less project) is a hard error rather than a silent fallback. Claude Code subagent worktrees: not yet. |
