...
| `context-watermark` | hook (`UserPromptSubmit`, `PostToolUse`) | Warns when context crosses a soft or hard watermark and nudges toward `/handoff` then `/clear` or `/compact`. The lines scale with the lead model's context window as a cap — `min(120k, 60% of the window)` and `min(160k, 80%)`, times a repo-size factor — so a small-window model is nudged before it overflows and a 1M-window one is still nudged inside the measured degradation band. On `PostToolUse` it also watches a delegated **worker**, which gets a soft line at half the session's and no hard tier: the only move a worker has is to wrap up and report. Fails open; never blocks. |
