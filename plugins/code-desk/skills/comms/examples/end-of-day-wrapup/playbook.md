...
skill writes the file. Toolchain `deliver.py` (bare `slides.json`, no type yet), lighter than the morning brief
(4-5 slides); audio optional (~1-2 min). Folder slug `-eod-wrapup`.

> Assumptions to confirm with the owner: daily PM cadence; deliver.py; audio optional. No prior
