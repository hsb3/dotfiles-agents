#!/usr/bin/env python3
"""gen_opencode.py — assemble the opencode dist lane from primitives-core (ADR 0008 W4).
...
membership and the translation.yaml capability matrix into the committed, drift-guarded lane
dist/opencode/:
  ...
  agents/<id>.md          transform — frontmatter remapped per the cc-to-opencode mapping:
                          ...
                          CC-only keys (effort) dropped, maxTurns -> steps; body verbatim
...
Deterministic and stdlib-only. Two modes:

  python3 scripts/gen_opencode.py            regenerate dist/opencode/ in place
  python3 scripts/gen_opencode.py --check    build to a temp dir, diff vs committed, exit 1
"""
...
DIST = os.path.join(REPO, "dist", "opencode")
...
// mcp entries from the roster would render here; none are rostered yet.
{
...
def main(argv):
    ...
    if args.check:
        tmp = tempfile.mkdtemp(prefix="gen-opencode-check-")
        try:
            problems = build(tmp, entries, translation)
            if not os.path.isdir(DIST):
                problems.append("dist/opencode: missing (run `make build` and commit)")
            elif not _identical(tmp, DIST):
                problems.append("dist/opencode: drift — committed copy != freshly generated")
        finally:
        ...
        problems = build(DIST, entries, translation)
    if problems:
        print(f"✗ opencode lane — {len(problems)} problem(s):")
        for p in problems:
            print(f"  - {p}")
        return 1
    n_skills = len([d for d in os.listdir(os.path.join(DIST, "skills"))]) if os.path.isdir(
        os.path.join(DIST, "skills")) else 0
    mode = "check clean" if args.check else "regenerated"
    print(f"✓ opencode lane {mode} — dist/opencode ({n_skills} skills + agents + installer)")
    return 0
