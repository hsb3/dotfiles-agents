...
`primitives-core/skills/<id>/README.md` if that source exists — additive only, mirroring
gen_marketplace's bundle-README mechanism. The README travels with its skill (ADR 0017 §4),
so the same file also rides along inside `skills/<id>/` via the verbatim skill copy.
...
def _copy_standalone_readme(skill_id, proot):
    """Copy primitives-core/skills/<skill_id>/README.md -> <proot>/README.md if the source
    exists (the README travels with its skill, ADR 0017 §4). Additive only, mirroring
    gen_marketplace._copy_bundle_readme — never fails the build. Placed at the wrapper ROOT
    (a sibling of skills/); the same file also rides inside skills/<id>/ via the verbatim
    skill copy the byte-identity invariant (verify_wrappers) compares against source."""
    src = os.path.join(SKILLS_DIR, skill_id, "README.md")
