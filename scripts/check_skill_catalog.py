#!/usr/bin/env python3
"""Standalone skill-catalog eligibility + drift guard (D4).
...
Claude-Code-only distribution (R4): the sole client is `claude-code`; the opencode dimension
and its installer are not part of this rebuild.
...
Eligibility — a catalogued skill must satisfy ALL of:
  ...
  2. roster `requires` (if any) contains none of local-mcp / hosted-mcp / hooks — a
     ...
     hook. cli:/env: deps are advisory (surfaced by the installer), not disqualifying.
...
def sibling_reference(folder, this_id, other_ids):
    ...
    WITHOUT a `skills/` prefix or wikilink brackets isn't tripped. The declared-empty
    `depends_on_skills` field is the primary gate, with this heuristic as a backstop."""
...
def unresolved_markdown_links(folder):
    """Return a list of (file, target, why) for every relative markdown link that escapes the
    folder or dangles. Links are resolved relative to the file that CONTAINS them. External /
    anchor / absolute targets are skipped — only in-repo relative file references matter."""
    problems = []
...
def check_entry(entry, roster_by_id, all_skill_ids, bundle_ids, problems):
    ...
    if r is None:
        problems.append(f"[{cid}] catalog id has no roster entry (catalog<->roster drift)")
        return
    if r.get("type") != "skill":
    ...
    for c in entry.get("clients", []):
        if c not in roster_targets:
            problems.append(f"[{cid}] client `{c}` not in roster targets {roster_targets}")
...
def main():
    ...
    if problems:
        ...
        for p in problems:
            print(f"  - {p}")
        return 1
    print(f"✓ skill-catalog clean — {n} standalone skill(s), eligibility + drift + namespacing OK")
    return 0
